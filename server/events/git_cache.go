...
func (g *GitCacheManager) EnsureMirror(logger logging.SimpleLogging, cloneURL string, repoFullName string) string {
	...
	if mirrorPath == "" {
		logger.Warn("invalid repo name for mirror path: %q", repoFullName)
		return ""
	}
	...
	// cloneURL comes from the VCS webhook and is trusted (filtered by --repo-allowlist).
	// mirrorPath is validated by mirrorPath() to prevent path traversal.
	cmd := exec.CommandContext(ctx, "git", "clone", "--mirror", cloneURL, mirrorPath) // nolint: gosec
	output, err := cmd.CombinedOutput()
	if err != nil {
		logger.Warn("failed to create mirror for %s: %s: %s", repoFullName, err, string(output))
		os.RemoveAll(mirrorPath) // nolint: gosec // mirrorPath is validated by mirrorPath()
		return ""
	}
...
// mirrorPath returns the path for a repo's mirror clone within the cache directory.
// Returns "" if the repo name is invalid or would escape the cache directory (path traversal).
func (g *GitCacheManager) mirrorPath(repoFullName string) string {
	cleanRepo := filepath.Clean(repoFullName)
	if cleanRepo == "." || cleanRepo == "" || filepath.IsAbs(cleanRepo) {
		return ""
	}
	if cleanRepo == ".." || strings.HasPrefix(cleanRepo, ".."+string(filepath.Separator)) {
		return ""
	}

	base := filepath.Clean(g.CacheDir)
	candidate := filepath.Join(base, cleanRepo+".git")

	// Final safety check: ensure the resolved path is still under the cache dir
